...
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath() %>/css/center1.css">
   <style>
    ...
    <div class="c-container w650 m30">
           ...
           <%for(CenterDto centerDto : centerList){ %>
            ...
            <div class="flex-c-container c-list-listbox m10">
                ...
                <div class="c-list-area">
                    ...
                    <div class="center exercise-kinds m20">
                        <h4><%=eocDto.getEocExerciseName() %></h4>
                    </div>
               <%} %>
                   <!--좋아요 수-->
                    <div class="flex-c-container">
                        <h4 class="c-basic-like"><%=centerDto.getCenterLikeCount() %></h4>                       
                        <img src="<%=request.getContextPath()%>/images/center_dummy/basiclike.png" width="16px" height="16px">
                    </div>
                </div>
            </div>
            <%} %>
      ...
      <%if(p > 1){ %>
         <%if(isSearch){ %>
         <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=1&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&laquo;</a>
         <%} else { %>
         <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=1&s=<%=s%>">&laquo;</a>
         <%} %>
      <%} %>
      
      <%if(startBlock > 1){ %>
         <%if(isSearch){ %>
         <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=startBlock-1%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&lt;</a>
         <%} else { %>
         <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=startBlock-1%>&s=<%=s%>">&lt;</a>
         <%} %>
      <%} %>
      ...
      <%for(int i=startBlock; i <= endBlock; i++){ %>
         <%if(isSearch){ %>
            <%if(i == p){ %>
            <a class="active" href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=i%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>"><%=i%></a>   
            <%} else { %>
            <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=i%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>"><%=i%></a>
            <%} %>
         <%} else { %>
            <%if(i == p){ %>
            <a class="active" href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=i%>&s=<%=s%>"><%=i%></a>   
            <%} else { %>
            <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=i%>&s=<%=s%>"><%=i%></a>
            <%} %>
         <%} %>
      <%} %>
      ...
      <%if(endBlock < lastPage){ %>
         <%if(isSearch){ %>
         <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=endBlock+1%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&gt;</a>
         <%} else { %>
         <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=endBlock+1%>&s=<%=s%>">&gt;</a>
         <%} %>
      <%} %>
      
      <%if(p < lastPage){ %>
         <%if(isSearch){ %>
         <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=lastPage%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&raquo;</a>
         <%} else { %>
         <a href="<%=request.getContextPath()%>/center/mainList.jsp?p=<%=lastPage%>&s=<%=s%>">&raquo;</a>
         <%} %>
      <%} %>
   </div>
